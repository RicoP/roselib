import os
import time
import datetime

bindings = [
	#BINDING(events, void, push, (void * data, int size), (data, size) )
	# namespace type    name  args: [ [type, name], ... ]
	#[ "events", "void", "push", [ ["void*", "data"], ["int", "size"] ] ]
	[ "utils", "float", "add", [ ["float", "a"], ["float", "b"] ] ]
]

# create file file bindings.h.tmp
with open("bindings.h.tmp", "w") as f:
	f.write("#pragma once\n")
	f.write("\n")
	f.write("///////////////////////////////////////////////////\n")
	f.write("// AUTOGENERATED: \n")
	f.write("// DO NOT EDIT THIS FILE\n")
	f.write("// EDIT: roselib/internal/generate_bindings.py\n")
	f.write("// Generated: " + datetime.datetime.now().strftime("%Y-%m-%d") + "\n")
	f.write("///////////////////////////////////////////////////\n")
	f.write("#include <roselib/context.h>\n")
	f.write("\n")

	#iterate over bindings and create the strings
	for binding in bindings:
		#typedef Pad (*input_pad_ft)();
		f.write("typedef ")
		f.write(binding[1])
		f.write(" (*")
		f.write("rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write("_ft)(")
		for arg in binding[3]:
			f.write(arg[0])
			f.write(" ")
			f.write(arg[1])
			f.write(", ") if arg != binding[3][-1] else f.write(");\n")
		
		#ROSE_EXPORT input_pad_ft c_input_pad;
		f.write("ROSE_EXPORT ")
		f.write("rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write("_ft c_")
		f.write("rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write(";\n\n")

	f.write("\n#ifdef __cplusplus\n")
	f.write("namespace rose {\n")
	for binding in bindings:
		#namespace rose {
		#	namespace input {
		#		pad pad() {
		#  		return c_input_pad();
		# 	}
		#}
		f.write("namespace ")
		f.write(binding[0])
		f.write(" {\n")
		f.write("  inline ")
		f.write(binding[1])
		f.write(" ")
		f.write(binding[2])
		f.write("(")
		for arg in binding[3]:
			f.write(arg[0])
			f.write(" ")
			f.write(arg[1])
			f.write(", ") if arg != binding[3][-1] else f.write(") {\n")
		f.write("    ")
		f.write("return "  if binding[1] != "void" else "")
		f.write("c_rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write("(")
		for arg in binding[3]:
			f.write(arg[1])
			f.write(", ") if arg != binding[3][-1] else f.write(");\n")
		f.write("  }\n")
		f.write("}\n")
	f.write("}\n")
	f.write("#endif\n")

	#guard for ROSE_ENGINE_INTERNAL
	f.write("\n#ifdef ROSE_ENGINE_INTERNAL\n")
	#for each binding 
	#print float g_rose_utils_add(float a, float b);
	for binding in bindings:
		f.write(binding[1])
		f.write(" ")
		f.write("g_rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write("(")
		for arg in binding[3]:
			f.write(arg[0])
			f.write(" ")
			f.write(arg[1])
			f.write(", ") if arg != binding[3][-1] else f.write(");\n")
	f.write("#endif\n\n")


	f.write("#if defined(ROSE_ENGINE_INTERNAL) && defined(ROSE_INJECT_BINDINGS) \n")
	f.write("//TODO: create unix binding.\n")
	f.write("inline void inject_bindings(HMODULE handle) { \n")
	#iteartor over bindings and create the string
	#{
	#	auto fun = (input_pad_ft*)(GetProcAddress(lib, "c_input_pad"));
	#	if(GetLastError() == 0) {
	#		*fun = g_input_pad;
	#	} else {
	#		fprintf( stderr, "Warning: Couldn't find function %s \n", "c_input_pad");
	#	}
	#}
	for binding in bindings:
		f.write("  {")
		f.write("\n    auto fun = (")
		f.write("rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write("_ft*)(GetProcAddress(handle, \"c_")
		f.write("rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write("\"));\n")
		f.write("    if(GetLastError() == 0) {\n")
		f.write("      *fun = ")
		f.write("g_rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write(";\n    } else {\n")
		f.write("      fprintf(stderr, \"Warning: Couldn't find function %s \\n\", \"c_")
		f.write("rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write("\");\n    }\n")
		f.write("  }\n")
		
	f.write("}\n")
	f.write("#endif\n\n")

	f.write("#if defined(ROSE_BINDINGS_IMPL) \n")
	for binding in bindings:
		#static rose_input_pad_ft rose_input_pad = 0;
		f.write("static ")
		f.write("rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write("_ft c_")
		f.write("rose_")
		f.write(binding[0])
		f.write("_")
		f.write(binding[2])
		f.write(" = 0;\n")
	f.write("#endif \n")

	f.close()
	os.replace("bindings.h.tmp", "bindings.h")